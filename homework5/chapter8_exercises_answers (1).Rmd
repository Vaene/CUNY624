...
first_hilo <- function(fc, level = 95){
  level_name <- paste0(level, "%")
  alpha <- (1 - level / 100) / 2

  dist_col <- names(fc)[vapply(fc, function(x){
    length(x) > 0 && inherits(x[[1]], "distribution")
  }, logical(1))][1]

  qs <- quantile(fc[[dist_col]][[1]], probs = c(alpha, 1 - alpha), na.rm = TRUE)

  tibble(
    .mean = fc$.mean[1],
    !!paste0(level_name, "_lower") := unname(qs[1]),
    !!paste0(level_name, "_upper") := unname(qs[2])
  )
}
...
```{r ex1-pi}
pigs_sigma <- augment(pigs_fit) %>% summarise(s = sd(.resid, na.rm = TRUE)) %>% pull(s)
pigs_manual_pi <- report_pi(pigs_fc$.mean[1], pigs_sigma)
pigs_r_pi <- first_hilo(pigs_fc, 95) %>% select(.mean, `95%_lower`, `95%_upper`)

bind_cols(
  tibble(method = "Manual: yhat +/- 1.96*s"),
  pigs_manual_pi
) %>%
  bind_rows(
    pigs_r_pi %>% transmute(method = "R/fable 95% interval", lower = `95%_lower`, upper = `95%_upper`)
  ) %>%
  kable(digits = 2)
```
...
exports_acc2 %>% select(.model, RMSE, AICc, MAE, MAPE) %>% arrange(RMSE) %>% kable(digits = 3)
...
bind_rows(
  tibble(model = "ANN manual", lower = ann_pi_manual$lower, upper = ann_pi_manual$upper),
  tibble(model = "ANN R", lower = ann_pi_r$`95%_lower`, upper = ann_pi_r$`95%_upper`),
  tibble(model = "AAN manual", lower = aan_pi_manual$lower, upper = aan_pi_manual$upper),
  tibble(model = "AAN R", lower = aan_pi_r$`95%_lower`, upper = aan_pi_r$`95%_upper`)
) %>% kable(digits = 3)
```
...
```{r ex6-acc}
accuracy(gdp_fit) %>% select(.model, RMSE, AICc, MAPE) %>% arrange(AICc) %>% kable(digits = 3)
```
...
accuracy(gas_fit) %>% select(.model, RMSE, AICc, MAPE) %>% arrange(AICc) %>% kable(digits = 3)
```
...
```{r ex10f}
bind_rows(
  accuracy(aus_trips_dcmp_fit),
  accuracy(aus_trips_ets)
) %>%
...
best_trips_model <- bind_rows(
  accuracy(aus_trips_dcmp_fit),
  accuracy(aus_trips_ets)
) %>% arrange(RMSE) %>% slice(1) %>% pull(.model)

preferred_trips_fit <- bind_rows(aus_trips_dcmp_fit, aus_trips_ets) %>% select(all_of(best_trips_model))
...
nz_arrivals <- aus_arrivals %>%
  filter(Origin == "New Zealand") %>%
  select(Quarter, Arrivals)

nz_train <- nz_arrivals %>% filter(Quarter <= yearquarter("2010 Q3"))
nz_test  <- nz_arrivals %>% filter(Quarter > yearquarter("2010 Q3"))

autoplot(nz_arrivals, Arrivals) +
  labs(title = "Quarterly arrivals to Australia from New Zealand", x = NULL, y = "Arrivals")
```
...
nz_cv %>% select(.model, RMSE, MAE, MAPE) %>% arrange(RMSE) %>% kable(digits = 3)
```
...
cement_mse %>% select(.model, MSE) %>% arrange(MSE) %>% kable(digits = 3)
```
...
compare_three_models <- function(data, value, test_years = 3){
  ...
  n_per_year <- frequency(data[[as_name(idx)]])
  ...
  fit <- train %>%
    model(
      ...
      SNAIVE = SNAIVE(!!value),
      STL_ETS = decomposition_model(
        STL(box_cox(!!value, (train %>% features(!!value, guerrero) %>% pull(lambda_guerrero)))),
        ETS(season_adjust)
      )
    )

  fc <- forecast(fit, new_data = test)
  accuracy(fc, test) %>% select(.model, RMSE, MAE, MAPE) %>% arrange(RMSE)
}
```
...
a10 <- PBS %>% filter(ATC2 == "A10") %>% group_by(Month) %>% summarise(Cost = sum(Cost), .groups = "drop")
h02 <- PBS %>% filter(ATC2 == "H02") %>% group_by(Month) %>% summarise(Cost = sum(Cost), .groups = "drop")
food <- aus_retail %>% filter(Industry == "Food retailing") %>% group_by(Month) %>% summarise(Turnover = sum(Turnover), .groups = "drop")
...
```{r ex14a}
trips_total <- tourism %>% group_by(Quarter) %>% summarise(Trips = sum(Trips), .groups = "drop")
...
ets_examples <- trips_total %>% model(Trips = ETS(Trips))
ets_examples %>% report()
