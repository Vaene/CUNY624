...
bind_rows(
  tibble(
    ...
    point_forecast = pigs_fc$.mean[1],
    interval = glue("[{round(pigs_manual_pi$lower, 2)}, {round(pigs_manual_pi$upper, 2)}]")
  ),
  tibble(
    method = "R/fable 95% interval",
    point_forecast = pigs_r_pi$.mean,
    interval = pigs_r_pi$interval
  )
) %>%
  kable(digits = 2)
```
...
```{r ex10f}
bind_rows(
  accuracy(aus_trips_dcmp_fit) %>% left_join(glance(aus_trips_dcmp_fit) %>% select(.model, AICc), by = ".model"),
  accuracy(aus_trips_ets) %>% left_join(glance(aus_trips_ets) %>% select(.model, AICc), by = ".model")
) %>%
...
```{r ex14a}
trips_total <- tourism %>% index_by(Quarter) %>% summarise(Trips = sum(Trips))
