...
const treeRegionNames = {
  InlVal: "Inland Valley",
  InterW: "Intermountain West",
  LoMidW: "Lower Midwest",
  MidWst: "Midwest",
  SoCalC: "Southern California Coast",
  InlEmp: "Interior Empire",
  SWDsrt: "Southwest Desert",
  PacfNW: "Pacific Northwest",
  NoCalC: "Northern California Coast",
  Tropic: "Tropical",
  GulfCo: "Gulf Coast",
  NMtnPr: "Northern Mountain Prairie",
  NoEast: "Northeast",
  TpIntW: "Temperate Interior West",
  Piedmt: "Piedmont",
  CenFla: "Central Florida",
  SacVal: "Sacramento Valley",
};
...
function initChartIfVisible(selector, initFn) {
  const slide = activeSlide();
  if (!slide || !slide.querySelector(selector)) return false;
  initFn();
  return true;
}
...
function showSlide(index) {
  ...
  if (window.TreeDemoSandbox?.mountAll) {
    window.TreeDemoSandbox.mountAll();
  }
...
function renderTreeSiteMix() {
  ...
  const innerW = width - margin.left - margin.right;
  const innerH = height - margin.top - margin.bottom;
  const root = d3.select(container).style("position", "relative");
  const tooltip = root
    .append("div")
  ...
  g.append("g")
    ...
    .call((sel) => sel.selectAll("text").attr("cursor", "help"))
    .call((sel) => sel.selectAll("text").each(function (d) {
      const fullName = treeRegionNames[d] || d;
      const text = d3.select(this);
      if (!text.select("title").size()) {
        text.append("title").text(fullName);
      }
    }));
  ...
  const dots = g.selectAll(".dot")
    ...
    .on("mouseenter", function (_event, d) {
      tooltip
        .style("opacity", 1)
        .html(`<strong>${treeRegionNames[d.region] || d.region}</strong><br>${(d.share * 100).toFixed(1)}% street share`);
      d3.select(this).attr("r", 6).attr("fill", palette.gold);
    })
    ...
    .on("mouseleave", function () {
      ...
      d3.select(this).attr("r", 3.8).attr("fill", palette.tree);
    });
  ...
  guides
    ...
    .on("mouseenter", function (_event, d) {
      tooltip
        .style("opacity", 1)
        .html(`<strong>${treeRegionNames[d.region] || d.region}</strong><br>${(d.share * 100).toFixed(1)}% street share`);
      d3.select(this).attr("stroke", palette.gold).attr("stroke-width", 2.5);
    })
    .on("mousemove", function (event) {
      const [xPos, yPos] = d3.pointer(event, container);
      tooltip.style("left", `${xPos}px`).style("top", `${yPos}px`);
    ...
    .on("mouseleave", function () {
      tooltip.style("opacity", 0);
      d3.select(this).attr("stroke", "rgba(31,41,51,0.12)").attr("stroke-width", 1);
    });
...
function renderTreeHeightProfile() {
  ...
  const width = container.clientWidth || 480;
  const height = 420;
  const margin = { top: 18, right: 20, bottom: 40, left: 68 };
  const innerW = width - margin.left - margin.right;
  const innerH = height - margin.top - margin.bottom;
  const svg = d3.select(container).append("svg").attr("viewBox", `0 0 ${width} ${height}`);
  const g = svg.append("g").attr("transform", `translate(${margin.left},${margin.top})`);
  ...
  g.append("g")
    ...
    .call((sel) => sel.selectAll("line").remove())
    .call((sel) => sel.selectAll("text").attr("fill", palette.muted).attr("font-size", 13))
    .call((sel) => sel.attr("transform", "translate(6,0)"));
...
function initSquirrelOverview() {
  renderSquirrelApplication();
}

function renderSquirrelApplication() {
  const id = "squirrelApplicationChart";
  if (chartState[id]) return;
  const container = document.getElementById(id);
  if (!container) return;

  const data = [
    { label: "No", count: 1588, share: 0.525, color: "#7e97b3", card: "white" },
    { label: "Yes", count: 1435, share: 0.475, color: "#7e97b3", card: "#a8c480" },
  ];

  const width = container.clientWidth || 560;
  const height = 560;
  const margin = { top: 28, right: 24, bottom: 214, left: 20 };
  const innerW = width - margin.left - margin.right;
  const innerH = 318;
  const svg = d3.select(container).append("svg").attr("viewBox", `0 0 ${width} ${height}`);
  const g = svg.append("g").attr("transform", `translate(${margin.left},${margin.top})`);
  ...
  svg.append("text")
    ...
    .attr("fill", palette.ink)
    .attr("font-size", 18)
    .text("Target distribution: Were the squirrels foraging?");
  ...
  g.append("line")
    ...
    .attr("y1", 0)
    .attr("y2", 0)
    .attr("stroke", "rgba(31,41,51,0.24)")
    .attr("stroke-width", 1);

  const x = d3.scaleBand().domain(data.map((d) => d.label)).range([48, innerW - 40]).padding(0.34);
  const y = d3.scaleLinear().domain([0, 1700]).range([innerH, 0]);
  const ticks = [500, 1000, 1500];
  ...
  g.append("g")
    ...
    .attr("y1", (d) => y(d))
    .attr("y2", (d) => y(d))
    .attr("stroke", "rgba(31,41,51,0.16)")
    .attr("stroke-width", 1);
  ...
  bars
    ...
    .attr("fill", "#fff")
    .attr("font-size", 14)
    .attr("stroke", "rgba(31,41,51,0.55)")
    .attr("stroke-width", 2)
    .attr("paint-order", "stroke")
    .text((d) => d.label);

  gsap.to(bars.select("rect").nodes(), {
    attr: {
      y: (i) => y(data[i].count),
      height: (i) => innerH - y(data[i].count),
    },
    duration: 0.9,
    stagger: 0.06,
    ease: "power2.out",
  ...
  const cardY = 382;
  const cardW = Math.min(220, (innerW - 30) / 2);
  const cardH = 150;
  const cardGap = innerW - cardW * 2;
  const cardX = [
    (innerW / 2 - cardGap / 2) / 2 - cardW / 2,
    (innerW / 2 + cardGap / 2) + (innerW / 2 - cardGap / 2) / 2 - cardW / 2,
  ];

  data.forEach((d, i) => {
    const group = svg.append("g").attr("transform", `translate(${cardX[i]},${cardY})`);
    group
      .append("rect")
      .attr("width", cardW)
      .attr("height", cardH)
    ...
    group
      ...
      .attr("fill", i === 0 ? "#6b6b6b" : "#5b5b5b")
      .attr("font-size", 16)
      .text(`${(d.share * 100).toFixed(1)}% of observations`);
  });

  chartState[id] = true;
}

function renderRandomForestGraphic() {
  const id = "randomForestGraphic";
  if (chartState[id]) return;
  const container = document.getElementById(id);
  if (!container) return;
  ...
  const leftW = panel.w * 0.5 - 14;
  const rightW = panel.w * 0.5 - 14;
  const leftX = panel.x;
  const rightX = panel.x + panel.w * 0.5 + 14;
  const titleY = panel.y + 34;
  const treeY = panel.y + 62;
  const histY = panel.y + 236;
  const cardY = panel.y + 344;
  ...
  svg.append("rect")
    ...
    .attr("width", panel.w)
    .attr("height", panel.h)
    .attr("fill", "rgba(239, 235, 228, 0.92)");
  ...
  svg.append("line")
    .attr("x1", panel.x + panel.w / 2)
    .attr("x2", panel.x + panel.w / 2)
    .attr("y1", panel.y + 48)
    .attr("y2", panel.y + panel.h - 88)
    .attr("stroke", "rgba(79,79,79,0.2)")
    .attr("stroke-dasharray", "4,5");

  function drawTree(group, x0, y0, scale, rootColor, leafColor, fillColor) {
    const nodes = [
      { x: x0 + 110 * scale, y: y0, r: 16, label: "A", fill: rootColor },
      { x: x0 + 68 * scale, y: y0 + 46 * scale, r: 15, label: "B", fill: leafColor },
      { x: x0 + 152 * scale, y: y0 + 46 * scale, r: 15, label: "C", fill: leafColor },
      { x: x0 + 28 * scale, y: y0 + 112 * scale, r: 13, label: "Leaf1", fill: fillColor },
      { x: x0 + 62 * scale, y: y0 + 112 * scale, r: 13, label: "Leaf2", fill: fillColor },
      { x: x0 + 116 * scale, y: y0 + 112 * scale, r: 13, label: "Leaf3", fill: fillColor },
      { x: x0 + 170 * scale, y: y0 + 112 * scale, r: 13, label: "Leaf4", fill: fillColor },
    ];
    const lines = [
    ...
    lines.forEach(([a, b]) => {
      group.append("line")
        .attr("x1", nodes[a].x)
        .attr("y1", nodes[a].y + nodes[a].r)
        .attr("x2", nodes[b].x)
        .attr("y2", nodes[b].y - nodes[b].r)
        .attr("stroke", "#333")
        .attr("stroke-width", 2);
    });
    nodes.forEach((n, i) => {
      ...
      group.append("text")
        .attr("x", n.x)
        .attr("y", n.y + (isLeaf ? 4 : 5))
        .attr("text-anchor", "middle")
        .attr("fill", "#2d2d2d")
        .attr("font-size", isLeaf ? 8 : 14)
        .attr("font-weight", 700)
        .text(n.label);
    });
  }

  const leftTree = svg.append("g");
  drawTree(leftTree, leftX + 8, treeY, 1, "#e1b06f", "#d1a66d", "#eacb86");

  const forestTreeXs = [rightX + 12, rightX + 90, rightX + 168];
  forestTreeXs.forEach((x0, idx) => {
    drawTree(svg.append("g"), x0, treeY - 4, 0.78, "#aeb9bc", idx % 2 === 0 ? "#d9bf85" : "#c4d26b", "#ecd89f");
  });
  ...
  // Left histogram and warning callout
  const leftHistX = leftX + 28;
  const leftBaseY = histY + 70;
  const leftHistW = leftW - 60;
  const leftHistH = 90;
  const leftLine = d3.line()(Array.from({ length: 64 }, (_, i) => {
    const t = i / 63;
    const y = 0.2 * Math.exp(-((t - 0.12) ** 2) / 0.006)
      + 0.55 * Math.exp(-((t - 0.32) ** 2) / 0.015)
      + 0.75 * Math.exp(-((t - 0.55) ** 2) / 0.025)
      + 0.4 * Math.exp(-((t - 0.78) ** 2) / 0.01);
    return [leftHistX + t * leftHistW, leftBaseY - y * leftHistH];
  }));
  svg.append("line")
    .attr("x1", leftHistX)
    .attr("x2", leftHistX + leftHistW)
    .attr("y1", leftBaseY)
    .attr("y2", leftBaseY)
    .attr("stroke", "#3a3a3a")
    .attr("stroke-width", 1.5);
  svg.append("line")
    .attr("x1", leftHistX + 10)
    .attr("x2", leftHistX + 10)
    .attr("y1", leftBaseY)
    .attr("y2", leftBaseY - leftHistH - 18)
    .attr("stroke", "#3a3a3a")
    .attr("stroke-width", 1.5);
  svg.append("path")
    .attr("d", leftLine)
    .attr("fill", "rgba(216, 130, 100, 0.65)")
    .attr("stroke", "#a9674d")
    .attr("stroke-width", 1.5);
  [0.18, 0.48, 0.72].forEach((t) => {
    svg.append("line")
      .attr("x1", leftHistX + t * leftHistW)
      .attr("x2", leftHistX + t * leftHistW)
      .attr("y1", leftBaseY)
      .attr("y2", leftBaseY - leftHistH - 5)
      .attr("stroke", "rgba(169,103,77,0.28)")
      .attr("stroke-dasharray", "5,5");
  });
  svg.append("path")
    .attr("d", `M${leftHistX - 14},${leftBaseY - 5} L${leftHistX - 28},${leftBaseY + 24} H${leftHistX + 0} Z`)
    .attr("fill", "#cf7a2a")
    .attr("stroke", "#333")
    .attr("stroke-width", 1.2);
  svg.append("text")
  ...
  svg.append("text")
    ...
    .attr("fill", "#2d2d2d")
    .attr("font-size", 16)
    .text("One decision tree on training data");
  svg.append("text")
    .attr("x", leftX + 50)
    .attr("y", cardY + 58)
    .attr("fill", "#2d2d2d")
    .attr("font-size", 15)
    .text("Predicted values can vary greatly with");
  svg.append("text")
    .attr("x", leftX + 50)
    .attr("y", cardY + 76)
    .attr("fill", "#2d2d2d")
    .attr("font-size", 15)
    .text("small changes in data");

  // Right histogram and check callout
  const rightHistX = rightX + 30;
  const rightBaseY = histY + 70;
  const rightHistW = rightW - 60;
  const rightHistH = 76;
  const rightLine = d3.line()(Array.from({ length: 64 }, (_, i) => {
    const t = i / 63;
    const y = 0.12 * Math.exp(-((t - 0.12) ** 2) / 0.03)
      + 0.9 * Math.exp(-((t - 0.45) ** 2) / 0.04)
      + 0.3 * Math.exp(-((t - 0.63) ** 2) / 0.03);
    return [rightHistX + t * rightHistW, rightBaseY - y * rightHistH];
  }));
  svg.append("line")
    .attr("x1", rightHistX)
    .attr("x2", rightHistX + rightHistW)
    .attr("y1", rightBaseY)
    .attr("y2", rightBaseY)
    .attr("stroke", "#3a3a3a")
    .attr("stroke-width", 1.5);
  svg.append("line")
    .attr("x1", rightHistX + 12)
    .attr("x2", rightHistX + 12)
    .attr("y1", rightBaseY)
    .attr("y2", rightBaseY - rightHistH - 12)
    .attr("stroke", "#3a3a3a")
    .attr("stroke-width", 1.5);
  svg.append("path")
    .attr("d", rightLine)
    .attr("fill", "rgba(177, 195, 146, 0.7)")
    .attr("stroke", "#768066")
    .attr("stroke-width", 1.5);
  [0.24, 0.53, 0.82].forEach((t) => {
    svg.append("line")
      .attr("x1", rightHistX + t * rightHistW)
      .attr("x2", rightHistX + t * rightHistW)
      .attr("y1", rightBaseY)
      .attr("y2", rightBaseY - rightHistH - 4)
      .attr("stroke", "rgba(118,128,102,0.25)")
      .attr("stroke-dasharray", "5,5");
  });
  svg.append("line")
    .attr("x1", rightHistX + rightHistW * 0.5)
    .attr("x2", rightHistX + rightHistW * 0.5)
    .attr("y1", rightBaseY - rightHistH - 30)
    .attr("y2", rightBaseY - rightHistH - 56)
    .attr("stroke", "#98a18d")
    .attr("stroke-width", 8)
    .attr("marker-end", "url(#rfArrow)");
  svg.append("line")
    .attr("x1", rightHistX + rightHistW * 0.5)
    .attr("x2", rightHistX + rightHistW * 0.5)
    .attr("y1", rightBaseY - rightHistH - 74)
    .attr("y2", rightBaseY - rightHistH - 100)
    .attr("stroke", "#98a18d")
    .attr("stroke-width", 8)
    .attr("marker-end", "url(#rfArrow)");
  ...
  svg.append("text")
    ...
    .attr("fill", "#2d2d2d")
    .attr("font-size", 16)
    .text("Combines multiple decision trees’ predictions");
  svg.append("text")
    .attr("x", rightX + 48)
    .attr("y", cardY + 58)
    .attr("fill", "#2d2d2d")
    .attr("font-size", 15)
    .text("Smoother, more stable predictions");
...
function initChartsForSlide(index) {
  ...
  initChartIfVisible("#squirrelOverviewChart", initSquirrelOverview);
  initChartIfVisible("#squirrelApplicationChart", renderSquirrelApplication);
  if (activeSlide.querySelector("#squirrelLocationMixViz") && activeSlide.querySelector("#squirrelPerchBoxViz")) {
    animateVisualBriefing(
      index,
      ["squirrelLocationMixViz", "squirrelPerchBoxViz"],
      [renderSquirrelLocationMix, renderSquirrelPerchBox]
    );
  }
  initChartIfVisible("#squirrelMethodsChart", initSquirrelMethods);
  initChartIfVisible("#squirrelBenchmarkChart", initSquirrelBenchmark);
  initChartIfVisible("#treeOverviewChart", initTreeOverview);
  initChartIfVisible("#randomForestGraphic", renderRandomForestGraphic);
  if (activeSlide.querySelector("#treeSiteMixViz") && activeSlide.querySelector("#treeHeightProfileViz")) {
    animateVisualBriefing(
      index,
  ...
  initChartIfVisible("#treeMethodsChart", initTreeMethods);
  initChartIfVisible("#treeBenchmarkChart", initTreeBenchmark);
  initChartIfVisible("#crossDatasetChart", initCrossDataset);
  initChartIfVisible("#wildfireUsMapViz", renderWildfireUsMap);
  initChartIfVisible("#importanceChart", initImportanceChart);
  initChartIfVisible("#boostSequenceViz", animateBoostFlow);
  initChartIfVisible("#rmseChart", initRmseChart);
  initChartIfVisible("#tradeoffChart", initTradeoffChart);
  maybeRunDeferredCharts();
}
