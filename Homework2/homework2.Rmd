...
myseries |>
  gg_lag(Turnover) +
  labs(title = "Lag plot: selected retail series (default lags = 9)", y = "Turnover")

myseries |>
  gg_lag(Turnover, lags = 1:12) +
  labs(title = "Lag plot: selected retail series (lags = 12)", y = "Turnover")
...
```{r graphics-exploration-lag24, fig.width=8, fig.height=16}
myseries |>
  gg_lag(Turnover, lags = 1:24) +
  facet_wrap(vars(.lag), ncol = 3) +
  labs(title = "Lag plot: selected retail series (lags = 1:24)", y = "Turnover")
```
...
explore_series <- function(data, value_col, title_prefix = "") {
  list(
    ...
    lag_default = tryCatch(
      data |> gg_lag({{ value_col }}) + labs(title = paste0(title_prefix, " — lag plot (default lags = 9)")),
      error = function(e) NULL
    ),

    lag_12 = tryCatch(
      data |> gg_lag({{ value_col }}, lags = 1:12) + labs(title = paste0(title_prefix, " — lag plot (lags = 12)")),
      error = function(e) NULL
    ),

    lag_24 = tryCatch(
      data |>
        gg_lag({{ value_col }}, lags = 1:24) +
        facet_wrap(vars(.lag), ncol = 3) +
        labs(title = paste0(title_prefix, " — lag plot (lags = 1:24)")),
      error = function(e) NULL
    ),
...
plots_emp$time; plots_emp$season; plots_emp$subseries; plots_emp$lag_default; plots_emp$lag_12; plots_emp$acf

plots_bricks <- explore_series(bricks_ts, Bricks, "Australian Bricks Production")
plots_bricks$time; plots_bricks$season; plots_bricks$subseries; plots_bricks$lag_default; plots_bricks$lag_12; plots_bricks$acf

plots_hare <- explore_series(hare_ts, value, "Hare Pelts")
plots_hare$time; plots_hare$season; plots_hare$subseries; plots_hare$lag_default; plots_hare$lag_12; plots_hare$acf

plots_pbs <- explore_series(pbs_h02, Cost, "PBS H02 Cost")
plots_pbs$time; plots_pbs$season; plots_pbs$subseries; plots_pbs$lag_default; plots_pbs$lag_12; plots_pbs$acf

plots_gas <- explore_series(gas_barrels, Barrels, "US Gasoline Barrels")
plots_gas$time; plots_gas$season; plots_gas$subseries; plots_gas$lag_default; plots_gas$lag_12; plots_gas$acf
```

```{r plots-gas-emp-hare-pbs-bricks-lag24, fig.width=8, fig.height=18}
plots_emp$lag_24
plots_bricks$lag_24
plots_hare$lag_24
plots_pbs$lag_24
