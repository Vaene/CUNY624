---
title: "FPP3 Exercises Homework 6"
author: "Randy Howk"
date: "`r Sys.Date()`"
output:
  pdf_document:
...
## Exercise 1(a) Use `ETS()` to estimate the equivalent SES model, report `alpha` and `l0`, and forecast four months ahead.
...
## Exercise 1(b) Compute a 95% prediction interval for the first forecast using `yhat +/- 1.96 s`, and compare it with R's interval.
...
# Exercise 2. Write your own SES function
...
**Answer:** Yes. Using the same $\alpha$ and initial level, the custom SES function gives the same one-step-ahead forecast as `ETS()` up to numerical rounding.

# Exercise 3. Modify the function to return SSE and optimize with `optim()`
...
# Exercise 4. One function that estimates and forecasts
...
**Answer:** The combined function returns the estimated $\alpha$, estimated $\ell_0$, and the one-step-ahead SES forecast in a single call.

# Exercise 5. Exports from one country in `global_economy`
...
## Exercise 5(a) Plot the series and discuss the main features.
...
## Exercise 5(b) Forecast with an `ETS(A,N,N)` model and plot the forecasts.
...
## Exercise 5(c) Compute the training-set RMSE.
...
## Exercise 5(d) Compare with `ETS(A,A,N)` and discuss the merits of the two methods.
...
## Exercise 5(e) Compare the forecasts. Which is best?
...
## Exercise 5(f) Compute a 95% prediction interval for the first forecast for each model using RMSE and compare with R.
...
# Exercise 6. Chinese GDP under different ETS options
...
# Exercise 7. Gas from `aus_production`
...
# Exercise 8. Retail series revisited
...
## Exercise 8(a) Why is multiplicative seasonality necessary?

**Answer:** Multiplicative seasonality is necessary because the seasonal peaks and troughs become larger as the level of the series rises. December peaks are much bigger in later years than in earlier years, so constant absolute seasonal effects are not appropriate.

## Exercise 8(b) Apply Holt-Winters multiplicative and experiment with damping.
...
## Exercise 8(c) Compare one-step RMSE. Which do you prefer?
...
## Exercise 8(d) Check whether the residuals from the best method look like white noise.
...
## Exercise 8(e) Compute test-set RMSE after training through 2010, and compare with seasonal naive.
...
# Exercise 9. STL + Box-Cox + ETS on the same retail series
...
# Exercise 10. Total domestic overnight trips across Australia
...
## Exercise 10(a) Plot the data and describe the main features.
...
## Exercise 10(b) Decompose using STL and obtain the seasonally adjusted series.
...
## Exercise 10(c) Forecast two years using additive damped trend on the seasonally adjusted data.
...
## Exercise 10(d) Forecast two years using Holt's linear method on the seasonally adjusted data.
...
## Exercise 10(e) Use `ETS()` to choose a seasonal model.
...
## Exercise 10(f) Compare RMSE for the ETS model and the STL decomposition models.
...
## Exercise 10(g) Compare the forecasts. Which seems most reasonable?
...
## Exercise 10(h) Check the residuals of the preferred model.
...
# Exercise 11. Arrivals from New Zealand
...
## Exercise 11(a) Make a time plot and describe the main features.
...
## Exercise 11(b) Hold out the last two years and forecast using Holt-Winters multiplicative.
...
## Exercise 11(c) Why is multiplicative seasonality necessary?
...
## Exercise 11(d) Forecast the two-year test set using four methods.
...
## Exercise 11(e) Which method is best? Does it pass the residual tests?
...
## Exercise 11(f) Compare the same four methods using time series cross-validation.
...
# Exercise 12. Cross-validation for Portland cement

## Exercise 12(a) Produce 1-year-ahead ETS and seasonal naive forecasts using a stretching window.

## Exercise 12(b) Compute the MSE of the 4-step-ahead errors and comment.
...
# Exercise 13. Compare `ETS()`, `SNAIVE()`, and `decomposition_model(STL, ...)` on five series
...
# Exercise 14. Use `ETS()` to select models for several series

## Exercise 14(a) Apply `ETS()` to total trips in `tourism`, the four `gafa_stock` series, and the lynx series in `pelt`. Does it always give good forecasts?
...
## Exercise 14(b) Find an example where it does not work well. Why?
...
# Exercise 15. Show that ETS(M,A,M) point forecasts equal Holt-Winters multiplicative forecasts
...
# Exercise 16. Show that the forecast variance for ETS(A,N,N) is `sigma^2 [1 + alpha^2 (h-1)]`
...
# Exercise 17. Write down 95% prediction intervals for ETS(A,N,N)
