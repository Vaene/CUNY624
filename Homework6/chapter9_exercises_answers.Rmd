...
first_hilo_tbl <- function(fc, level = 95) {
  ...
  h <- h %>% unpack_hilo(!!rlang::sym(hcol), names_sep = "_")
  lower_col <- paste0(hcol, "_lower")
  upper_col <- paste0(hcol, "_upper")
  tibble(
    point_forecast = as_num(h$.mean[1]),
    lower = as_num(h[[lower_col]][1]),
    upper = as_num(h[[upper_col]][1])
  )
}
